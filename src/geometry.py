...
def analyze_overhangs(shape, max_angle=45.0) -> List[Dict]:
    ...
    # find the lowest Z point — that's the build plate face, skip it
    ...
    for face in all_faces:
        try:
            ...
            # skip the absolute bottom face (build plate)
            centroid = face_centroid(face)
            if abs(centroid[2] - min_z) < 0.5:
                continue

            angle_below_horizontal = round(np.degrees(np.arcsin(abs(nz))), 2)

            if angle_below_horizontal > max_angle:
                loc = centroid
                if not any(nearby(loc, i['location']) for i in issues):
                    issues.append({
                        "issue_type": "overhang",
                        "location": loc,
                        "angle_degrees": angle_below_horizontal,
                        "severity": "critical" if angle_below_horizontal > 70 else "warning",
                        "fix": f"Reduce overhang to < {max_angle}° or add support material"
                    })
        except Exception:
            continue
    return issues
                ...
                if 0 < thickness < min_thickness:
                    ...
                    if not any(nearby(loc, i['location']) for i in issues):
                        issues.append({
                            ...
                            "fix": f"Increase wall thickness to at least {min_thickness}mm"
                        })
        except Exception:
            continue
    return issues
            ...
            if 0 < length < min_radius:
                ...
                if not any(nearby(loc, i['location']) for i in issues):
                    issues.append({
                        ...
                        "severity": "warning",
                        "fix": f"Add fillet radius of at least {min_radius}mm"
                    })
        except Exception:
            continue
    return issues
